...
//饼图
@property (nonatomic, strong) PieChartView *pieChartView1;
@property (nonatomic, strong) PieChartData *data1;
@property (nonatomic, strong) PieChartView *pieChartView2;
@property (nonatomic, strong) PieChartData *data2;
@property (nonatomic, strong) PieChartView *pieChartView3;
@property (nonatomic, strong) PieChartData *data3;
@property (nonatomic, strong) PieChartView *pieChartView4;
@property (nonatomic, strong) PieChartData *data4;
...
- (void)setCake{
    //创建饼状图
    self.pieChartView1 = [[PieChartView alloc] initWithFrame:CGRectMake(20, 320, SCR_W-40, 300)];
    [self.scrollView addSubview:self.pieChartView1];
    
    //基本样式
    [self.pieChartView1 setExtraOffsetsWithLeft:30 top:0 right:30 bottom:0];//饼状图距离边缘的间隙
    self.pieChartView1.usePercentValuesEnabled = YES;//是否根据所提供的数据, 将显示数据转换为百分比格式
    self.pieChartView1.dragDecelerationEnabled = YES;//拖拽饼状图后是否有惯性效果
    self.pieChartView1.drawSliceTextEnabled = YES;//是否显示区块文本
    //空心饼状图样式
    self.pieChartView1.drawHoleEnabled = YES;//饼状图是否是空心
    self.pieChartView1.holeRadiusPercent = 0.5;//空心半径占比
    self.pieChartView1.holeColor = [UIColor clearColor];//空心颜色
    self.pieChartView1.transparentCircleRadiusPercent = 0.52;//半透明空心半径占比
    self.pieChartView1.transparentCircleColor = [UIColor colorWithRed:210/255.0 green:145/255.0 blue:165/255.0 alpha:0.3];//半透明空心的颜色
    //实心饼状图样式
    //    self.pieChartView.drawHoleEnabled = NO;
    //饼状图中间描述
    if (self.pieChartView1.isDrawHoleEnabled == YES) {
        self.pieChartView1.drawCenterTextEnabled = YES;//是否显示中间文字
        //普通文本
        ...
        self.pieChartView1.centerAttributedText = centerText;
    }
    //饼状图描述
    self.pieChartView1.descriptionText = @"饼状图示例";
    self.pieChartView1.descriptionFont = [UIFont systemFontOfSize:10];
    self.pieChartView1.descriptionTextColor = [UIColor grayColor];
    //饼状图图例
    self.pieChartView1.legend.maxSizePercent = 1;//图例在饼状图中的大小占比, 这会影响图例的宽高
    self.pieChartView1.legend.formToTextSpace = 5;//文本间隔
    self.pieChartView1.legend.font = [UIFont systemFontOfSize:10];//字体大小
    self.pieChartView1.legend.textColor = [UIColor grayColor];//字体颜色
    self.pieChartView1.legend.position = ChartLegendPositionBelowChartCenter;//图例在饼状图中的位置
    self.pieChartView1.legend.form = ChartLegendFormCircle;//图示样式: 方形、线条、圆形
    self.pieChartView1.legend.formSize = 12;//图示大小
    //为饼状图提供数据
    self.data1 = [self setPieData];
    self.pieChartView1.data = self.data1;
    //设置动画效果
    [self.pieChartView1 animateWithXAxisDuration:1.0f easingOption:ChartEasingOptionEaseOutExpo];
