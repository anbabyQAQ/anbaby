...
#define kbackgroundColor UIColorFromRGB(0xc62828)
...
- (void)setDataSource:(id<DOPDropDownMenuDataSource>)dataSource {
    ...
    for (int i = 0; i < _numOfMenu; i++) {
        ...
        CALayer *bgLayer = [self createBgLayerWithColor:kbackgroundColor andPosition:bgLayerPosition];
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height andSuperView:(UIView *)view{
    ...
    self = [self initWithFrame:CGRectMake(origin.x, origin.y, 100, height)];
    if (self) {
        _origin = origin;
        ...
        //lefttableView init
        _leftTableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x, 0, self.frame.size.width/2, 0) style:UITableViewStylePlain];
        ...
        //righttableView init
        _rightTableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x + self.frame.size.width/2, 0, self.frame.size.width/2, 0) style:UITableViewStylePlain];
        ...
        _buttomImageView = [[UIImageView alloc]initWithFrame:CGRectMake(origin.x, 0, SCREEN_WIDTH, kButtomImageViewHeight)];
        _buttomImageView.image = [UIImage imageNamed:@"icon_chose_bottom"];
        
        //self tapped
        self.backgroundColor = UIColorFromRGB(0xc62828);
...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (show) {
        if (haveItems) {
            _leftTableView.frame = CGRectMake(self.origin.x, 0, SCREEN_WIDTH/2, 0);
            _rightTableView.frame = CGRectMake(self.origin.x + SCREEN_WIDTH/2, 0, SCREEN_WIDTH/2, 0);
            [self.superview addSubview:_leftTableView];
            [self.superview addSubview:_rightTableView];
        } else {
            _leftTableView.frame = CGRectMake(self.origin.x, 0, SCREEN_WIDTH, 0);
            _rightTableView.frame = CGRectMake(self.origin.x + SCREEN_WIDTH/2, 0, SCREEN_WIDTH/2, 0);
            [self.superview addSubview:_leftTableView];
            
        }
        _buttomImageView.frame = CGRectMake(self.origin.x, 0, SCREEN_WIDTH, kButtomImageViewHeight);
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
                _leftTableView.frame = CGRectMake(self.origin.x, 0, SCREEN_WIDTH/2, tableViewHeight);
                
                _rightTableView.frame = CGRectMake(self.origin.x + SCREEN_WIDTH/2, 0, SCREEN_WIDTH/2, tableViewHeight);
            } else {
                _leftTableView.frame = CGRectMake(self.origin.x, 0, SCREEN_WIDTH, tableViewHeight);
            }
            _buttomImageView.frame = CGRectMake(self.origin.x, CGRectGetMaxY(_leftTableView.frame)-2, SCREEN_WIDTH, kButtomImageViewHeight);
        }];
    } else {
        [UIView animateWithDuration:0.2 animations:^{
            if (haveItems) {
