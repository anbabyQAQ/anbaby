...
#import "PutBodyHttpThread.h"

@implementation PutBodyHttpThread
...
-(void)onPrev{
    ...
    [LogUtil write:@"params" value:self.params];
...
-(void)http{
    ...
    [request setQueryStringSerializationWithStyle:AFHTTPRequestSimpleStringBodyStyle];//自定义文本
    [request setQueryStringSerializationWithBlock:^NSString*(NSURLRequest * request, id value, NSError *__autoreleasing* error){
        return [NSString stringWithFormat:@"%@",value];//value是传递的params，表示将value按照什么形式序列化
    }];
    if(self.header){
        
        for(id key in [self.header allKeys]){
            
            [request setValue:[self.header valueForKey:key] forHTTPHeaderField:key];
        }
    }
