...
+ (NSString*) keyForUrl:(NSString*) url andSuffix:(NSString*) suffix{
    NSString * nail=[TSFileCache suffixForUrl: url];
    
    if([DataUtil isEmptyString:suffix]){
       nail=[TSFileCache suffixForUrl: url];
    }else{
        nail=suffix;
    }
    ...
    NSString* key=[NSString stringWithFormat:@"%@.%@",url,nail];
    
    return key;

}

+ (NSString*) suffixForUrl:(NSString*) url{
  
    NSArray *a= [url componentsSeparatedByString:@"?"];
    NSString* body= [a objectAtIndex:0];
    NSArray*b=[body componentsSeparatedByString:@"."];
